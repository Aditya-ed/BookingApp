        ...
        <div>
            <AccountNav/>
                    <form onSubmit={savePlace}>
                        ...
                        {preInput('Address','Add address to this place')}
                        <input type="text" value={address} onChange={ev=>setAddress(ev.target.value)} placeholder="address" />
                        
                        
                        {preInput('Photos','Add some photos!')}
                        <PhotosUploader addedPhotos={addedPhotos} onChange={setAddedPhotos}/>
                        {preInput('Description','Add description of this place')}
                        <textarea value={description} onChange={ev=>setDescription(ev.target.value)}/>
                        
                        
                        {preInput('Perks','Select the perks provided')}
                        <div className="grid gap-2 grid-cols-2 md:grid-cols-3 lg:grid-cols-6">
                            <Perks selected={perks} onChange={setPerks}/>
                        </div>
                        {preInput('ExtraInfo','Additional Info')}
                        <textarea value={extraInfo} onChange={ev=>setExtraInfo(ev.target.value)}/>

                        {preInput('Check In & Out time','Remember to have some time window for cleaning the room between guests')}
                        <h2 className="text-xl mt-4"></h2>
                        <p className="text-gray-500 text-sm"></p>
                        <div className="grid gap-2 grid-cols-3 md:grid-cols-4">
                            <div>
                            <h3 className="mt-2 -mb-1">Check in Time</h3>
                            <input type="text" value={checkIn} onChange={ev=>setCheckIn(ev.target.value)} placeholder="15:00"/>
                            </div>
